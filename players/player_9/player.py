	...
	def propose_item(self, history: list[Item]) -> Item | None:
		...
		for item in self.memory_bank:
			...
			if len(item.subjects) == 1:
				...
				if history_subjects.get(item.subjects[0], 0) == 0 and history_subjects.get(item.subjects[1], 0) == 0:
					coherence_bonus -= 0
				elif history_subjects.get(item.subjects[0], 0) == 1 and history_subjects.get(item.subjects[1], 0) == 1:
					coherence_bonus += 0.5
				elif history_subjects.get(item.subjects[0], 0) >= 2 and history_subjects.get(item.subjects[1], 0) >= 2:
					coherence_bonus += 1
			...
			if len(history) >= 1:
				if history[-1] is None:
					...
					if len(item.subjects) == 1:
						...
						if item.subjects[0] not in history_subjects and item.subjects[1] not in history_subjects:
							freshness_bonus += 1
			...
			if len(history) >= 3:
				...
				for s in history_subjects:
					if s in item.subjects:
						if history_subjects[s] >= 3:
			...
			total_bonus = coherence_bonus + importance_bonus + freshness_bonus + nonmonotonousness_bonus + individual_bonus

			item_scores.append((item, total_bonus, coherence_bonus, importance_bonus, freshness_bonus, nonmonotonousness_bonus, individual_bonus))
