...
class DebugLogger:
    ...
    def log(self, level: int, category: str, message: str, data: Optional[Dict] = None):
    ...
    def log_item_evaluation(self, item: Item, scores: Dict[str, float], total_delta: float, rank: int):
        """Log item evaluation and scoring."""
        if not config_module.DEBUG_ITEM_EVALUATION:
            return
        
        self.log(2, "ITEM_EVAL", 
                f"Item '{item.text[:30]}...' - Δ={total_delta:.3f} (rank {rank})")
        
        if self.level >= 2:
            self.log(2, "ITEM_EVAL", "Score breakdown:", {
    ...
    def log_decision_summary(self, final_decision: Optional[Item], reason: str, 
                           strategy_used: str, confidence: float = 0.0):
        ...
        self.log(1, "DECISION", 
                f"Final: {decision_text} | Strategy: {strategy_used} | Reason: {reason}")
