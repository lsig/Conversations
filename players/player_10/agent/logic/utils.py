...
def find_first_proposer_tier(counts_by_pid: dict[uuid.UUID, int], exclude_self: uuid.UUID) -> list[uuid.UUID]:
    """
    ...
    tier = [
        pid for pid, count in counts_by_pid.items()
        if count == min_count and pid != exclude_self
    ]
...
def pick_safe_keepalive_item(
    memory_bank: Iterable[Item],
    seen_item_ids: Set[uuid.UUID],
    history: Sequence[Item | None]
) -> Item | None:
    """
