...
class ResultsAnalyzer:
    ...
    def plot_altruism_comparison(self, save_path: str = None):
        ...
        altruism_groups = df.groupby('altruism_prob').agg({
        ...
        ax1.errorbar(altruism_probs, mean_scores, yerr=std_scores, 
        ...
        ax2.errorbar(altruism_probs, mean_p10_scores, yerr=std_p10_scores,
    ...
    def plot_parameter_heatmap(self, param1: str, param2: str, metric: str = 'total_score', 
                              save_path: str = None):
    ...
    def print_detailed_analysis(self):
        ...
        print(f"Unique configurations: {df.groupby(['altruism_prob', 'tau_margin', 'epsilon_fresh', 'epsilon_mono']).ngroups}")
        ...
        print(f"Conversation Length - Mean: {df['conversation_length'].mean():.1f}, Std: {df['conversation_length'].std():.1f}")
        print(f"Early Termination Rate: {df['early_termination'].mean():.2f}")
        
        # Best configurations
        print(f"\n=== TOP 10 CONFIGURATIONS ===")
        top_configs = df.groupby(['altruism_prob', 'tau_margin', 'epsilon_fresh', 'epsilon_mono']).agg({
            'total_score': ['mean', 'std', 'count'],
            'player10_score': 'mean'
        }).round(3)
        ...
        for i, (config, row) in enumerate(top_configs.iterrows(), 1):
            ...
            print(f"{i:2d}. Altruism: {altruism:.1f}, Tau: {tau:.2f}, "
        ...
        # Altruism analysis
        print(f"\n=== ALTRUISM ANALYSIS ===")
        altruism_stats = df.groupby('altruism_prob').agg({
        ...
        for prob in sorted(df['altruism_prob'].unique()):
            ...
            print(f"Altruism {prob:.1f}: Total={stats[('total_score', 'mean')]:.2f}±{stats[('total_score', 'std')]:.2f}, "
...
def main():
    ...
    parser.add_argument('--plot', choices=['altruism', 'heatmap', 'distributions'], 
                       default='altruism', help='Type of plot to create')
