...
if __name__ == "__main__":
    ...
    for field_name, field_info in Args.__dataclass_fields__.items():
        parser.add_argument(f"--{field_name}", type=type(getattr(Args(), field_name)), 
                          default=getattr(Args(), field_name), help=field_info.metadata.get('help', ''))
    ...
    for global_step in range(args.total_timesteps):
        ...
        if terminated or truncated:
            print(f"global_step={global_step}, episodic_return={episode_reward:.3f}, episode_length={episode_length}")
