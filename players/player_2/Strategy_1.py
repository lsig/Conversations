...
class Strategy1(BaseStrategy):
	...
	def propose_item(self, player: Player, history: list[Item]) -> Item | None:
		...
		player.sub_to_item = dict(sorted(player.sub_to_item.items(), key=lambda x: len(x[1]), reverse=True))
		...
		if turn_nr > 1 and history[-1] == player.last_proposed_item:
			...
			if last_proposed_subjects in player.sub_to_item and len(player.sub_to_item[last_proposed_subjects]) != 0:
				# print(f"Still have items with subjects {player.sub_to_item[last_proposed_subjects]}") 
				most_valuable_item = max(player.sub_to_item[last_proposed_subjects], key=lambda item: self._get_overall_score(item, player))
		...
		if turn_nr == 1 or turn_nr > 1 and history[-1] is None:
			...
			most_valuable_item = max(coherent_items, key=lambda item: self._get_overall_score(item, player))
			player.last_proposed_item = most_valuable_item
		...
		if turn_nr > 1:
			...
			for subs, _ in context_subs_sorted:
				...
				if len(subs) == 1:
					...
					items_with_subs.extend([item for items_subs, items in player.sub_to_item.items() if subs[0] in items_subs for item in items])
	...
	def _get_subjects_counts_sorted(self, items: list[Item], player: Player) -> list:
		...
		subs_sorted_by_count = sorted(((subs if isinstance(subs, tuple) else (subs,), count) for subs, count in subs_count.items()), 
								key=lambda x: (-x[1], -len(player.sub_to_item[x[0]]) if x[0] in player.sub_to_item else 0))
	...
	def _get_overall_score(self, item: Item, player: Player) -> float:
		...
		item_bonuses = [1 - (player.preferences.index(sub)) / player.subject_num for sub in item.subjects]
